...
Graph.Prototype.Permission = {
	...
	attemptConnectionBetween: function(node1,node2,errorRepository) {
		...
		if (this.areConnectionRulesSatisfied(node1,node2,errorRepository) && maxConnectionsNotBreached) {
			let connection = new Graph.Connection(node1,node2);
	...
	,attemptConnectionDeletion: function(connection,errorRepository) {
		if (this.areDisonnectionRulesSatisfied(connection,errorRepository)) {
			...
			if (node1.handler != undefined) {
				node1.handler.onDisconnectionFromNode(node1,node2,connection);
			}
			if (node2.handler != undefined) {
				node2.handler.onDisconnectionFromNode(node2,node1,connection);
			}
			return true;
		}
...
Graph.Prototype.Node = {
	...
	,deleteNode: function(errorRepository) {
		...
		for (const oneSidedConnection of this.oneSidedConnections) {
			permission = this.masterType.getPermissionToNodeType(oneSidedConnection.toNode.masterType);
			if (permission.areDisonnectionRulesSatisfied(oneSidedConnection.masterConnection,errorRepository) == false) {
...
////// Connection
////
Graph.Connection = function(node1,node2) {
